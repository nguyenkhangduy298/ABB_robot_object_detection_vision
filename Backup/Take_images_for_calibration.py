...
CamR = cv2.VideoCapture(4)  # 0 -> Right Camera
CamL = cv2.VideoCapture(2)  # 2 -> Left Camera

statue = CamL.set(cv2.CAP_PROP_BRIGHTNESS, 50)
print(statue)
...
while True:
    ...
    retR, cornersR = cv2.findChessboardCorners(grayR, (chessWidth, chessHeight),
                                               None)  # Define the number of chess corners (here 9 by 6) we are looking for with the right Camera
    retL, cornersL = cv2.findChessboardCorners(grayL, (chessWidth, chessWidth), None)  # Same with the left camera
    # cv2.imshow('imgR',frameR)
    # cv2.imshow('imgL',frameL)
    cv2.imshow("Com", np.hstack((frameL, frameR)))
    cv2.imshow("Gray", np.hstack((grayL, grayR)))
    ...
    if (retR == True) & (retL == True):
        ...
        cv2.drawChessboardCorners(grayR, (chessWidth, chessHeight), corners2R, retR)
        cv2.drawChessboardCorners(grayL, (chessWidth, chessHeight), corners2L, retL)
        ...
        if cv2.waitKey(0) & 0xFF == ord('s'):  # Push "s" to save the images and "c" if you don't want to
            ...
            print('Images ' + str(t) + ' saved for right and left cameras')
            cv2.imwrite('chessboard-R' + str_id_image + '.png',
                        frameR)  # Save the image in the file where this Programm is located
